struct Statistics	// check
{
	...
	inline Statistics(GLdouble sx , GLdouble sy , GLdouble sxy , GLdouble sx2 , unsigned int sn , GLdouble sa , GLdouble sb , GLdouble sc , GLdouble scenter_x , GLdouble scenter_y)
		:x(sx),y(sy),xy(sxy),x2(sx2),n(sn),a(sa),b(sb),c(sc),center_x(scenter_x),center_y(scenter_y)
	{
		// empty body
	} // end Statistics constructor

	inline Statistics &operator=(const Statistics &original)
	{
		x = original.x;
		y = original.y;
		xy = original.xy;
